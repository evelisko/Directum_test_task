...
class ObjectLocalization():
    ...
    def __init__(self, config_path, logger):
        ...
        try:
            ...
            self.image_operations = image_operations.ImageOperations(logger, config_path)
            self.orign_width = 0
            self.orign_height = 0
    ...
    def predict_interpretation(self, predict, class_name):
        ...
        if predict[0][:, 0].max() > self.threshold:
            ...
            bbx = self.image_operations.combined_bb(predict[0], predict[1], class_name, self.threshold)
            if (bbx[2] > self.min_object_size) and (bbx[3] > self.min_object_size):
                ...
                position['position'] = {'left': bbx[0], 'top': bbx[1], 'width': bbx[2],
                                        'height': bbx[3]}  # координаты объекта
    ...
    def get_objects_localization(self, image):
        ...
        try:
            ...
            self.logger.write(
                f'ObjectLocalization_Exception: Ошибка интерпретации модели! Не удалось выполнить предскание. {str(e)}')
            print(
                f'ObjectLocalization_Exception: Ошибка интерпретации модели! Не удалось выполнить предскание. {str(e)}')
